...
class exp:
    def __init__(self) -> None:
        ...
        # /home/ghosnp/project/fix_space/OpenPCDet/data/kitti/training/velodyne/
        # /home/ghosnp/project/fix_space/OpenPCDet/data/kitti/training/label_2/
        ...
        self.pcd_dir = '/home/ghosnp/project/fix_space/OpenPCDet/data/kitti/training/velodyne/'
        self.txt_dir = '/home/ghosnp/project/fix_space/OpenPCDet/data/kitti/training/label_2/'
    ...
    def fit_hyper_plane(self, model):
        ...
        return model.vfe.pfn_layers[0].norm.weight
    ...
    def iter_get_s4(self):
        ...
        norm_weight = self.fit_hyper_plane(model)
        norm_weight = norm_weight.detach().cpu().numpy()
        name_ = []
        dist_ = []
        pcc_ = []
        tick = time.time()
        ...
        for pcd_path in glob.glob(pcd_dir + '*.bin'):
            ...
            if (len(new_valid_features) != 0):
                for f in new_valid_features:
                    norm_f = (f - np.mean(f)) / np.std(f)
                    pcc = self.cal_PCC(norm_f, norm_weight)
                    pccs.append(pcc)
                    name_.append(pcd_path.split('/')[-1].split('.')[0])
                    pcc_.append(pcc)
                dists = np.abs(self.clf.decision_function(new_valid_features))
                dist_.extend(dists)
                mean_dist = np.mean(dists)
                mean_pcc = np.mean(pccs)
                print('mean_dist: ', mean_dist, 'mean_pcc: ', mean_pcc)
                print(len(dists), len(pccs))
            # name_.append(pcd_path.split('/')[-1].split('.')[0])
            
            # dist_ append 
            
            
            # dist_.append(mean_dist)
            # pcc_.append(mean_pcc)


        return name_, dist_, pcc_
    
if __name__ == '__main__':
    s4_exp = exp()
    name_, dist_,pcc_ = s4_exp.iter_get_s4()
    csv_path = './s4_score.csv'
    data = {'name': name_, 'dist': dist_, 'pcc': pcc_}
